...
class StoreInventory:
    ...
    def add_item_to_inventory(self, item_id, item_name, item_price, item_quantity):
        ...
        df_final = df_id.join([df_name, df_price, df_quantity])
        self.df = df_final
        return self.df

    def remove_item_by_name(self, item_name):
        self.df = self.df[self.df.ITEM_NAME != item_name]

    def remove_item_by_id(self, item_id):
        self.df = self.df[self.df.ITEM_ID != item_id]

    def update_item_price(self, item_id, item_new_price):
        self.df.loc[self.df["ITEM_ID"] == item_id, "ITEM_PRICE"] = item_new_price

    def update_item_quantity(self, item_id, item_new_quantity):
        self.df.loc[self.df["ITEM_ID"] == item_id, "ITEM_QUANTITY"] = item_new_quantity

    def update_item_name(self, item_id, item_new_name):
        self.df.loc[self.df["ITEM_ID"] == item_id, "ITEM_NAME"] = item_new_name
